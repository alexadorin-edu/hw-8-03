# Домашнее задание к занятию "`GitLab`" - `Алексей Дорин`
...
Новое содержание gitlab-ci.yml:
stages:
  - build
  - test
...
build_job:
  ...
  tags:
    - docker
  # Сборка запускается всегда, не дожидаясь тестов
...
test_job:
  ...
  rules:
    - changes:
        - "*.go"      # Тесты запускаются только при изменении .go файлов
